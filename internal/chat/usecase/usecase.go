...
func (c *ChatUseCase) ProcessMessage(ctx context.Context, req chat.ProcessMessageRequest) (chat.ProcessMessageResponse, error) {
	...
	msg := fmt.Sprintf(c.responseMap[rnd], responseDB.Stations, responseDB.PeopleFlow)
...
func (c *ChatUseCase) Cache() {
	...
	c.responseMap = map[int]string{
		0: "Похоже вы искали сколько было человек на станции %[1]s - по моим данным это %[2]d  за день",
		1: "За запрошенное время на станции %[1]s прошло %[2]d человек",
		2: "Нашла данные по вашему запросу - в это время на станции %[1]s пассажиропоток составил %[2]d человек",
		3: "Удалось найти информацию по вашему запросу: на станции %[1]s - поток составил %[2]d",
		4: "Исходя из запрошенных данных поток составил %[2]d на станции  %[1]s",
		5: "Судя по моим данным, поток на этой станции составил %[2]d",
	}
	c.responseMapMtx.Unlock()
}
